...
# Aqui estão as dores reais que levam aos seus produtos
CASOS_REAIS = [
    ...
    {"queixa": "Meu filho não come fruta nem verdura direito, vive comendo besteira. Tô preocupada com as vitaminas dele.", "produto_alvo": "Suprabio A-Z ou Kids (se tiver)"},
]
...
if colaborador != "Clique...":
    if not st.session_state.cenario:
        # BOTÃO AGORA SORTEIA DO BANCO DE DADOS (ZERO ERRO DE API AQUI)
                ...
                if not MODELO_NOME and not CONEXAO_OK:
                    ...
                    with st.spinner("O Treinador está analisando..."):
                        try:
                            ...
                            1. Fez sondagem? (Perguntou sintomas antes de ofertar?)
                            2. Criou conexão? (Não foi robô?)
                            3. Indicou o produto correto ({st.session_state.produto_alvo}) focando no BENEFÍCIO?
